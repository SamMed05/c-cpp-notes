...
Variables in programming are kind of a container with a label that holds information. 
...
These limits are defined in the [`<limits>` header](https://docs.microsoft.com/en-us/cpp/c-language/cpp-integer-limits?view=msvc-170) 
...
`auto` must be initialized with some value.
...
What `decltype` does is recognize the type of a variable inserted and assume that type. In 
the above case, `x` is the name of the variable passed and its type is `int`, so the whole 
...
because now the type that `decltype` assumes is the same as `y`, which is `float`.
...
variable or the `Num` variable. These are three different identifiers identifying three 
different variables.
...
Remember that is not the variable `num` that gives its value to 5, but it's 5 that is being 
stored inside `num`!
...
That process is called *initialization*, and if we assign a memory location and a value to 
the variable we have *defined* it.

#### Unexpected behaviors

Usually it's better to initialize the variable at a certain value, to avoid unexpected 
behaviors.
